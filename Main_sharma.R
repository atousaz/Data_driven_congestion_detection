...
# normal congestion delay -------------------------------------------------
  ...
  for (timespan in timespan_SPEED_HEAT){
    h_filter<-elbow_cut_data_RC%>%filter(char_time==timespan)%>%select(Speed)
  ...
  #calculating average speed and % for RC congestions daily--------------------------------------
  ...
  for (day in as.character(days)){
    ...
    cut_h<-unique(d_filter$hour)
    for (hh in cut_h){
      h_filter<-d_filter%>%filter(hour==hh)%>%select(Speed)
      avg_speed<-apply(h_filter[1],2,mean)
      sd_speed<-apply(h_filter[1],2,sd)
      #con_SD<-apply(h_filter[2],2,sd)
      avg_c<-cbind(sts,day,hh,avg_speed,sd_speed)
...
write.csv(speed_heat,"../Main/speed_heat.csv")
write.csv(speed_template_daily,"../Main/speed_template_daily.csv")
